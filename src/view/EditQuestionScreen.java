...
public class EditQuestionScreen extends JFrame {
    ...
    private ImageIcon uploadedPhoto = null;
    private File file = null;

    public EditQuestionScreen(Question question, String saveFileName) {
        ...
        btnFotoYukle = new JButton(new ImageIcon(getClass().getResource("/images/upload.png")));
        ...
        btnFotoYukle.addActionListener(e -> {
            ...
            if(result == JFileChooser.APPROVE_OPTION) {
                ...
                lblPhotoPreview.setText(file.getName());
                uploadedPhoto = new ImageIcon(file.getAbsolutePath());
                JOptionPane.showMessageDialog(this,
                        "Fotoğraf seçildi: " + file.getName());
            }
        });
        ...
        // Radiobutton action: Çoktan seçmeli / Açık uçlu
    ...
    private void fillFormWithQuestion(Question question) {
        ...
        if(diff.equalsIgnoreCase("Kolay")){
            rbKolay.setSelected(true);
        } else if(diff.equalsIgnoreCase("Orta")){
            rbOrta.setSelected(true);
        } else if(diff.equalsIgnoreCase("Zor")){
            rbZor.setSelected(true);
        }

        // Fotoğraf
        if(question instanceof MCQuestion) {
            MCQuestion mc = (MCQuestion) question;
            if(mc.getPhotoIcon() != null){
                rbFotoEvet.setSelected(true);
                photoPanel.setVisible(true);
                lblPhotoPreview.setText(mc.getPhotoName());
                uploadedPhoto = question.getPhotoIcon();
                file = question.getPhotoFile();
            } else {
                rbFotoHayir.setSelected(true);
                photoPanel.setVisible(false);
            }
        } else if(question instanceof OEQuestion){
            OEQuestion oe = (OEQuestion) question;
            if(oe.getPhotoIcon() != null){
                rbFotoEvet.setSelected(true);
                photoPanel.setVisible(true);
                lblPhotoPreview.setText(oe.getPhotoName());
                uploadedPhoto = question.getPhotoIcon();
                file = question.getPhotoFile();
            } else {
                rbFotoHayir.setSelected(true);
        ...
        if(question instanceof MCQuestion){
            ...
            MCQuestion mc = (MCQuestion)question;
            optionAText.setText(mc.getOptionA());
            optionBText.setText(mc.getOptionB());
            optionCText.setText(mc.getOptionC());
            optionDText.setText(mc.getOptionD());

            char correct = mc.getCorrectOption();
            if(correct == 'A'){
            ...
            OEQuestion oe = (OEQuestion) question;
            openAnswerField.setText(oe.getCorrectAnswer());
        }
    }
    ...
    private void saveQuestionAction() {
        ...
        if(rbCoktan.isSelected()){
            ...
            MCQuestion q;
            if(rbFotoEvet.isSelected() && uploadedPhoto != null){
                ImageIcon photoIcon = uploadedPhoto;
                q = new MCQuestion(questionText, difficulty,
                        optionA, optionB, optionC, optionD, correctOption, photoIcon, file);
            } else {
                q = new MCQuestion(questionText, difficulty,
            ...
            if(rbFotoEvet.isSelected() && uploadedPhoto != null){
                ImageIcon photoIcon = uploadedPhoto;
                q = new OEQuestion(questionText, difficulty, correctAnswer, photoIcon, file);
            } else {
                q = new OEQuestion(questionText, difficulty, correctAnswer);
            }
            saveQuestionToFile(q);
        }
    }

    private void saveQuestionToFile(domain.Question question) {
        ...
        File file = new File(questionsDir, saveFileName);
        ...
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(file))) {
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Soru güncellenirken hata oluştu.", "Hata", JOptionPane.ERROR_MESSAGE);
        }
